...
int genVDPoissonSampling(int *acqmask, float fovy, float fovz, int sky, 
			 ...
			 int cutcorners, long ran_seed, float pp) 
{
	...
	/* jtamir: change ran_seed functionality 2014.10.02 */
	/* Unless specified, always make the same pattern for a given input */
	ran_seed = ran_seed ? ran_seed : 1251046800;  /* Aug 23, 2009 10am */
		...
		for (i = 0; i < sky; i++) {
			for (j = 0; j < skz; j++) {
				...
				if (pp==10)
					...
					rrval = (1.0 - 1.0/sqrt(pp))/(1.0-r0) * mr * (R[i*skz + j] - 1.0) + mr - 1;
				else
					rrval = 0;
				/*peng ^^^^^^^^^^*/
